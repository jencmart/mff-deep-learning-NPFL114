...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        rois = inputs[0]
        mrcnn_class = inputs[1] # class probs
        mrcnn_bbox = inputs[2]
        image_meta = inputs[3]
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, log_dir):
        """
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # Subsamples proposals
            # and generates target outputs for training
            ...
            mrcnn_class_logits, mrcnn_class_probs, mrcnn_bbox = \
                fpn_classifier_graph(rois, mrcnn_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)

            mrcnn_mask_head = build_fpn_mask_graph(rois,
                                              mrcnn_feature_maps,
                                              input_image_meta,
            ...
            mask_loss = KL.Lambda(lambda x: mrcnn_mask_loss_graph(*x), name="mrcnn_mask_loss")([target_mask, target_class_ids, mrcnn_mask_head])
            ...
            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       mrcnn_class_logits, mrcnn_class_probs, mrcnn_bbox, mrcnn_mask_head,
            ...
            # normalized coordinates
            mrcnn_all_bbox_detection = DetectionLayer(config, name="mrcnn_detection")([rpn_rois, mrcnn_class_probs, mrcnn_bbox, input_image_meta])

            # Create masks for detections
            detection_boxes = KL.Lambda(lambda x: x[..., :4])(mrcnn_all_bbox_detection)
            mrcnn_mask_head = build_fpn_mask_graph(detection_boxes, mrcnn_feature_maps,
                                              input_image_meta,
                                              config.MASK_POOL_SIZE,
                                              config.NUM_CLASSES,
                                              train_bn=config.TRAIN_BN)
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors],
                             [mrcnn_all_bbox_detection, mrcnn_class_probs, mrcnn_bbox,
                              mrcnn_mask_head, rpn_rois, rpn_class, rpn_bbox],
                             name='mask_rcnn')
    ...
    def find_last(self):
        ...
        dir_names = next(os.walk(self.log_dir))[1]
        ...
        # Pick last directory
        dir_name = os.path.join(self.log_dir, dir_names[-1])
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.log_dir, "{}{:%Y%m%dT%H%M}".format(self.config.NAME.lower(), now))
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        for i in range(N):
            ...
            full_mask = utils.unmold_mask(masks[i], boxes[i], original_image_shape) # resize to size of image
