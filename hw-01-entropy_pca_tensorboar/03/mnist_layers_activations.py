...
if __name__ == "__main__":
    ...
    model.add(tf.keras.layers.Flatten())  #todo  - a `tf.keras.layers.Flatten()` layer

    for k in range(args.layers): # `args.layers` number of fully connected hidden layers   `tf.keras.layers.Dense()`
        if args.activation == "none":
            model.add(tf.keras.layers.Dense(units=args.hidden_layer, activation=None))
            # print("adding NONE")
        if args.activation == "relu":
            model.add(tf.keras.layers.Dense(units=args.hidden_layer, activation=tf.nn.relu))
            # print("adding RELU")
        if args.activation == "tanh":
            model.add(tf.keras.layers.Dense(units=args.hidden_layer, activation=tf.nn.tanh))
            # print("adding TANH")
        if args.activation == "sigmoid":
            model.add(tf.keras.layers.Dense(units=args.hidden_layer, activation=tf.nn.sigmoid))
            # print("adding SIGMOID")

    model.add(tf.keras.layers.Dense(units=MNIST.LABELS, activation=tf.nn.softmax)) # todo units=MNIST.LABELS, activation=tf.nn.softmax
