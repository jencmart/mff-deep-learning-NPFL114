...
class Network:
    def __init__(self, args):
        ...
        # `self.generator` as a Model, which
        # - takes vectors of [args.z_dim] shape on input
        # - applies args.generator_layers dense layers with ReLU activation
        ...
        x = input
        for i, fc_units in enumerate(args.generator_layers):
            x = tf.keras.layers.Dense(units=fc_units, activation=tf.nn.relu, name="gen_fc_"+str(i))(x)
        x = tf.keras.layers.Dense(units=MNIST.H * MNIST.W * MNIST.C, activation=tf.nn.sigmoid, name="gen_out_fc")(x)
        output = tf.keras.layers.Reshape([MNIST.H, MNIST.W, MNIST.C],  name="gen_out")(x)
        self.generator = tf.keras.Model([input], [output], name="gan_generator")

        # `self.discriminator` as a Model, which
        # input [MNIST.H, MNIST.W, MNIST.C]
        # flattens
        # args.discriminator_layers dense layers with ReLU activation,
        # - applies output dense layer with 1 output and a suitable activation function
        input = tf.keras.Input(shape=[MNIST.H, MNIST.W, MNIST.C], name="disc_in")
        x = input
        x = tf.keras.layers.Flatten()(x)
        for i, fc_units in enumerate(args.discriminator_layers):
            x = tf.keras.layers.Dense(units=fc_units, activation=tf.nn.relu, name="disc_fc_"+str(i))(x)
        output = tf.keras.layers.Dense(units=1, activation=tf.nn.sigmoid, name="disc_out")(x)  # todo sigmoid ?
