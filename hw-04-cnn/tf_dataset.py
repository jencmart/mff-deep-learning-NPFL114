...
if __name__ == "__main__":
    ...
    # The structure of a single example is inferred from the argument of `from_tensor_slices`
    # -- in our case we want each example to be a pair of `(input_image, target_label)`,
    # so we need to pass a pair `(data["images"], data["labels"])` to `from_tensor_slices`.
    ...
    def train_augment(image, label):
        ...
        image = tf.image.resize(image, [tf.random.uniform([], minval=CIFAR10.H, maxval=CIFAR10.H + 12, dtype=tf.int32), tf.random.uniform([], minval=CIFAR10.W, maxval=CIFAR10.W + 12, dtype=tf.int32)])
