...
# todo tady se udelaji veci pro batch
def compute_anchor_targets_bbox(
        anchors,
        image_group,
        annotations_group,
        num_classes,
    ...
    for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
        if annotations['bboxes'].shape[0]:
            ...
            # todo -- toto my pocitame v bbox_training
    ...
    # regression batch (batch_size, # anchors ? , )
...
def guess_shapes(image_shape, pyramid_levels):
    ...
    """
    image_shape = np.array(image_shape[:2])  # [(w,h), (w,h) .. ]
...
def anchors_for_shape(
        ...
        pyramid_levels=None,
        anchor_params=None,
        shapes_callback=None,  # shapes_callback=self.compute_shapes
):
    ...
    for idx, p in enumerate(pyramid_levels):
        # anchors for one pyramid level
        anchors = generate_anchors(base_size_on_pyramid_level=anchor_params.sizes[idx],
...
# .         size of pyramid level                  [0.5, 1, 2]    [2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0)]
def generate_anchors(base_size_on_pyramid_level=16, ratios=None, scales=None):
    """
    ...
    anchors = np.zeros((num_anchors, 4))  # vsechny jsou nula na zacatku
    # scale base_size
    anchors[:, 2:] = base_size_on_pyramid_level * np.tile(scales, (2, len(ratios))).T  # two same columns [scale 0 , scale 1 , scale 2 , scale 0 ,. ... ]
...
# to je stejene jako bbox_to_fast_rcnn
